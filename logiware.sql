...
-- Tabela adicionada para a atividade Somativa --
/* Tabela de inspeção de qualidade*/
...
-- Novas partes adicionadas para a Somativa --
/* Procedure para registrar as inspeções de qualidade */
DELIMITER $$
CREATE PROCEDURE registrar_inspecao_qualidade (
    IN p_id_produto INT,
    IN p_data DATE,
...
BEGIN
    INSERT INTO Inspecao_Qualidade (
        id_produto, data_inspecao, responsavel, resultado, observacoes
    ) VALUES (
        p_id_produto, p_data, p_responsavel, p_resultado, p_observacoes
    );
END $$
DELIMITER ;

/* VIEW para consultar as inspeções realizadas por produtos com detalhes */
CREATE VIEW v_inspecao_por_produto AS
SELECT
    P.id AS id_produto,
    P.descricao,
    I.id_inspecao,
    ...
    I.observacoes
FROM Produto P
JOIN Inspecao_Qualidade I ON P.id = I.id_produto;
...
/* Exemplo de funcionamento da procedure de registro de inspeção de qualidade */
CALL registrar_inspecao_qualidade(
    2,
    '2025-06-01',
    'João da Silva',
    'Aprovado',
    'Produto sem problemas com lacre intacto.'
);
SELECT * FROM Inspecao_Qualidade;

/* Exemplo de funcionamento da VIEW de consulta das inspeções por produtos */
SELECT * FROM v_inspecao_por_produto;
